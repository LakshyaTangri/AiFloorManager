...
| MISSING | All of S01 below M01 — boot chain, OS image, runtime; M05 listeners and vault; M07 decoder; M08 model execution; M13 desired/reported state; M14 OTA; F04 identity/enrolment/anti-clone; F17 kill switch; M17 GUI; factory provisioning; runbooks and field procedures |
| CONFLICTING | See §2 |
| UNSPECIFIED | See §3 |
...
| 6 | M11 | P2P sessions with no store access | 5 | **Host-side done in this change** (certificates minted by the T2 broker and only verified here, domain/signature/device/window ordering, purpose binding, renewal at 75% of TTL, 72 h then read-only until attestation *and* entitlement refresh, open/close chained with duration, expiry closing abandoned sessions, `live_onsite` labelling, no service at all on L1). Remaining: the mTLS server on the LAN serving the cloud OpenAPI paths with X.509 client certificates, a real T2 session broker and attestation exchange, and a port scan of an L1 build |
